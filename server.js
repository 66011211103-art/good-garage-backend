...
app.post('/api/auth/login', (req, res) => {
  ...
  db.query(
    ...
    async (err, results) => {
      if (err) return res.json({ success: false, message: 'เกิดข้อผิดพลาด' });
      // ✅ แก้กลับตามคำขอ — แยกข้อความอีกครั้งให้บอกชัดว่าอีเมล/ยังไม่ได้สมัคร กับ
      // รหัสผ่านผิด เป็นคนละกรณีกัน (ไม่มีคอลัมน์แยกระหว่าง "พิมพ์อีเมลผิด" กับ "ยังไม่
      // เคยสมัคร" ในระบบ — ทั้งสองกรณีตรวจได้แค่ว่า "หาอีเมลนี้ในระบบไม่เจอ" เหมือนกัน
      // เลยใช้ข้อความเดียวคือ "ไม่พบบัญชีนี้" ครอบคลุมทั้งสองแบบ)
      if (results.length === 0) {
